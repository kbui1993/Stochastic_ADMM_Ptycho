...
%              iteration
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [z, objective, best_z] = official_stochastic_iso_nonblind_ADMM(N_obj, probe, dp, ind_b, param, mode)
    ...
    if strcmp(param.z_update_mode, 'ePIE')
        PIE_step_size = 1/max(max(abs(probe).^2));
    elseif strcmp(param.z_update_mode, 'rPIE')
        peak = max(max(abs(probe).^2));
        PIE_step_size=1./((1-param.gamma)*abs(probe).^2+param.gamma*peak);
    elseif strcmp(param.z_update_mode, 'PIE')
        peak = max(max(abs(probe).^2));
        PIE_step_size = abs(probe)./(peak*(abs(probe).^2+param.gamma*peak^2));
    else
        PIE_step_size = 1;
    end
    ...
    for i = 1:param.maxiter
        ...
        for j=1:inner_iter
            ...
            if strcmp(mode, 'AGM')
                for k = 1:size(batch_updateOrder,2)
                    ...
                    u(:,:,new_idx) = prox_AGM(z(ind_b(new_idx,3):ind_b(new_idx,4), ind_b(new_idx,1):ind_b(new_idx,2)),...
                        probe, dp(:,:,new_idx), Lambda(:,:,new_idx), beta1);
            ...
            %otherwise fidelity term is IPM
                ...
                for k = 1:size(batch_updateOrder,2)
                    new_idx = batch_updateOrder(k);
                    u(:,:,new_idx) = prox_IPM(z(ind_b(new_idx,3):ind_b(new_idx,4), ind_b(new_idx,1):ind_b(new_idx,2)), probe, dp(:,:,new_idx), Lambda(:,:,new_idx), beta1);
                    count_mat(ind_b(new_idx,3):ind_b(new_idx,4), ind_b(new_idx,1):ind_b(new_idx,2)) = ...
                        count_mat(ind_b(new_idx,3):ind_b(new_idx,4), ind_b(new_idx,1):ind_b(new_idx,2)) + 1;
                end
            end
            ...
            %threshold
            [vx, vy] = shrink2(temp1, temp2, lambda/beta2);
            ...
            for k = 1:size(batch_updateOrder,2)
                ...
                new_obj_roi = conj(probe).*probe.*obj_roi;
                
                %compute  the gradient of the  quadratic fidelity term for F(P_jz): 
                %(\beta_1/2)*\|u_j^t - F(P_jz) + w_j^t/beta_1\|^2
                update_term = conj(probe).*fftshift(ifft2(ifftshift(u(:,:,new_idx)+Lambda(:,:,new_idx)/beta1)))-...
                    new_obj_roi;
        ...
        objective(i) = obj_value(z, probe, ind_b, dp, mode);
...
function val = obj_value(z, probe, ind_b, dp, mode)
    %get number of masks
    n = size(ind_b,1);
    ...
    TV_val = sqrt(abs(Dx_val).^2 + abs(Dy_val).^2);
